...
1.  Replicate the published analysis and evaluate the impact of key design choices in the methodology.
2.  Develop alternative methodology to answer the same research questions, as instructed below.
...
Most of the lifting for this analysis was already done in class. As noted, the published analysis excluded 192 unidentified variables; since we do not know which variables these were, we applied the methods in the paper to all variables. In addition, we made some simplifications for ease of presentation -- specifically, repeated RF fitting, forward selection in the logistic regression model, and repeated estimation of AUROC for the final classifier(s) were omitted. As such, our results will not match those in the paper exactly.
...
2.  Temporarily remove the outlier trimming from preprocessing and do some exploratory analysis of outlying values. Are there specific *subjects* (not values) that seem to be outliers? If so, are outliers more frequent in one group or the other?
